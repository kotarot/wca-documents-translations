...
- 2i) 競技中、競技者はスタックマットタイマーおよびストップウォッチ以外の電子機器や音響機器（例：携帯電話、MP3 プレーヤー、録音機器、追加照明）を使用してはならない。
    ...
    - 2i3) 競技者は携帯電話をポケットの中に入れることができる。ただし、他の方法で携帯電話と接触していないと明確に分かる場合に限る。
- 2j) WCA 代理人は特定の種目において競技者を失格させることができる。
    - 2j1) 競技者が何らかの理由で失格となると、その種目における試技を続ける資格はなくなる。
        - 2j1a) その競技者があるラウンドで失格になるまでに、既にそのラウンドで少なくとも 1 回の試技を始めていた場合、その種目の残りの試技の結果は全て DNF と記録される。その競技者が試技を始めていない場合（すなわち記録がない場合または DNS の記録しかない場合）、記録は残さない。
...
- 7h) 競技エリアには 1 つ以上の競技者待機場所がなければならない（代替規則：[規則 Y6a](regulations:regulation:Y6a)）。
    ...
    - 7h2) 定義：競技者待機場所とは、グループ内の競技者が競技台に呼ばれるために待機する場所である。
...
- 9t) 大会の終了後に、ある競技者の結果に変更（例：記録入力ミス、ペナルティの見逃し）が生じたことでその競技者が次のラウンドに進むべきでなかった場合（またはそのラウンドに参加することが許可されるべきでなかった場合）は、その競技者のそれ以降のラウンドの結果は全て削除されなければならない。
    - 9t1) これによってあるラウンドが[規則 9p1](regulations:regulation:9p1) に従わなくなった場合、その他の競技者の結果は削除してはならない。
    - 9t2) これによってあるラウンドが[規則 9m1](regulations:regulation:9m1)、[規則 9m2](regulations:regulation:9m2) または [規則 9m3](regulations:regulation:9m3) に従わなくなった場合、それ以降のラウンドはいずれも削除してはならない。
    - 9t3) ある競技者の結果に変更が生じ、別の競技者が次のラウンドに進むべきでなかった場合は、後者の競技者の結果は削除してはならない。
...
- 11g) WCA 代理人は、あらゆるインシデントに対応するため、WCA 大会規則と WCA ガイドラインのコピーを大会役員と競技者に閲覧可能な状態にしておかなければならない（例：印刷物、デジタル、またはインターネットを介したアクセス）。
...
- A6) 試技の終了：
    ...
    - A6e) 競技者はタイマーを止めた後、ジャッジが完成状態を確認するまで、パズルに触れたり回転させたりしてはならない。
        - A6e1) タイマーを止めた後にパズルを回転させた場合の罰則：試技の失格（DNF）。
        - A6e2) タイマーを止めた後にパズルに触れたがパズルを回転させなかった場合の罰則：+2 秒ペナルティ。例外：競技者がタイマーを止めた直後にわずかな時間パズルに触れたがパズルの状態が変わらなかった場合、ジャッジの裁量によりこのタイムの罰則を免除することができる（参照：[規則 11f](regulations:regulation:11f)）。
    - A6f) 競技者とジャッジの両方が記録用紙への結果の記録を完了するまで、競技者はタイマーをリセットしてはならない。罰則：ジャッジの裁量で試技の失格（DNF）。
...
- A7) 結果の記録：
    ...
    - A7c) 競技者は結果を確認して、結果が正しく、完全で、正しい書式で、明らかに読めると同意するため記録用紙に名前（またはイニシャル）で署名しなければならない。これにより試技が終了する。
        ...
        - A7c5) 署名の不足が見つかった場合は、大会が終了する前であれば WCA 代理人の裁量によりジャッジと競技者の両方、またはいずれか一方が署名を追記することができる。
...
- B2) 試技の開始：
    ...
    - B2c) 競技者は試技を開始しようとしている間にパズルやパズルの覆いに触れてはならない。罰則：+2 秒ペナルティ。
...
- B4) 目隠し段階：
    - B4a) 競技者は目隠しを着用し、目隠し段階を開始する。
        - B4a1) 競技者は最初の一手を始める前であれば、目隠しを取って記憶段階に戻ることができる。
    - B4b) 競技者は完全に目隠しを着用するまではパズルを回転させてはならない。罰則：試技の失格（DNF）。
    - B4c) 競技者が最初の一手を始めてからすぐに、ジャッジは競技者の顔とパズルの間が視野を遮るもの（例：紙や段ボール）で確実に遮られていることを確保しなければならない。
...
- C1) 基本的な進行手続きは[第A条](regulations:article:A)（スピード競技）に準じる。[第A条](regulations:article:A)の対応箇所にとってかわる追加規則は以下の通りである。
    - C1b) ソルブ中、競技者は片方の手以外の身体の部位でパズルを操作してはならない。罰則：試技の失格（DNF）。
        ...
        - C1b3) 競技者の片方の手以外の身体の部位がパズルに一時的に接触し、それが競技者の故意ではないかつパズルを回転させていない場合、ジャッジの裁量によりそれはパズルを操作したとはみなされない。
...
- H1) 基本的な進行手続きは[第B条](regulations:article:B)（目隠し競技）に準じる。[第B条](regulations:article:B)の対応箇所にとってかわる追加規則は以下の通りである（部分的な代替規則：[規則 Y7](regulations:regulation:Y7)）。
    ...
    - H1e) 競技者は記憶段階の間一個以上のパズルを回転させてはならない。罰則：試技の失格（DNF）。例外：WCA 代理人の裁量により、試技の失格とせず、これら個々のパズルを完成されていないものとすることができる。
- H2) WCA 代理人の裁量により、競技者は持参した競技面を覆うためのフラットな覆い（例：マットや布）を使用することができる。競技者がその使用を望む場合は、パズルの個数を申告するときに要求しなければならない（参照：[規則 H1a](regulations:regulation:H1a)）。
