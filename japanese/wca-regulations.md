...
- A6) 試技の終了：
    ...
    - A6e) 競技者はタイマーを止めた後、ジャッジが完成状態を確認するまで、パズルに触れたり回転させたりしてはならない。
        ...
        - A6e2) タイマーを止めた後にパズルを回転させずにパズルに触れた場合の罰則：+2 秒ペナルティ。例外：もし競技者がタイマーを止めた直後にパズルの状態を変えずにわずかな時間パズルに触れた場合、ジャッジの裁量によりこのタイムの罰則を免除することができる（参照：[規則 11f](regulations:regulation:11f)）。
...
- A7) 結果の記録：
    ...
    - A7c) 競技者は結果を確認して、結果が正しく、完全で、正しい書式で、明らかに読めると同意するため記録用紙に名前（またはイニシャル）で署名しなければならない。これにより試技が終了する。
        ...
        - A7c5) 署名に記入が無いことが大会の終了前に見つかった場合はジャッジと競技者の両方、またはいずれか一方は WCA 代理人の裁量で署名を書き加えることができる。
...
- B4) 目隠し段階：
    ...
    - B4c) 競技者が最初の一手を始めてからすぐに、ジャッジは競技者の顔とパズルの間が視野を遮るもの（例：紙や段ボール）で確実に遮られていることを保証しなければならない。
